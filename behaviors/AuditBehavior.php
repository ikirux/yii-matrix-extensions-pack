...
/**
 * AuditBehavior will automatically fill date and time related attributes.
 *
 * AuditBehavior will automatically fill date and time related attributes when the active record
 ...
 * respectively, so it is not required that you configure them. If you do not wish AuditBehavior
 ...
 *
 * AuditBehavior class file.
 * Although AuditBehavior attempts to figure out on it's own what value to inject into the timestamp attribute,
...
class AuditBehavior extends CActiveRecordBehavior {
	...
	/**
	 * @var mixed The name of the attribute to store the create user.  
	 */
	...
	/**
	 * @var mixed The name of the attribute to store the update user. 
	 */
	public $updateUser = 'update_time_id';	
	...
	public function beforeSave($event) {
		if ($this->getOwner()->getIsNewRecord() && ($this->createAttribute !== null)) {
			...
			$this->getOwner()->{$this->createUser} = Yii::app()->user->id;
		}
		...
		if ((!$this->getOwner()->getIsNewRecord() || $this->setUpdateOnCreate) && ($this->updateAttribute !== null)) {
			...
			$this->getOwner()->{$this->updateUser} = Yii::app()->user->id;
		}
	}
