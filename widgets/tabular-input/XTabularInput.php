 ...
 *
 * <pre>
 * $this->widget('path.to.XTabularInput', [
 *     'models' => $persons,
 ...
 *     return [
 *         'addTabularInputs' => [
 *             'class' => 'path.to.XTabularInputAction',
 *             'modelName' => 'Person',
 ...
 *     'addHtmlOptions' => ['class' => 'blue pill full-width'],
 *     'removeTemplate' => '<td>{link}</td>',
 *     'removeLabel' => Yii::t('ui', 'Delete'),
 *     'removeHtmlOptions' => ['class' => 'red pill'],
 * ]);
 * </pre>
 *
 ...
 * @author ikirux <ikirux@gmail.com> 
 * @version 1.1.0
 */
class XTabularInput extends CWidget
{
	 ...
	 */
	public $inputContainerHtmlOptions = [];
	/**
	...
	public function init()
	{
		...
		if (!isset($this->containerHtmlOptions['class'])) {
			$this->containerHtmlOptions = array_merge($this->containerHtmlOptions, ['class' => $this->containerCssClass]);
		} else {
			$this->containerHtmlOptions['class'] .= ' ' . $this->containerCssClass;
		}

		if (!isset($this->inputContainerHtmlOptions['class'])) {
			$this->inputContainerHtmlOptions = array_merge($this->inputContainerHtmlOptions, ['class' => $this->inputContainerCssClass]);
		} else {
			$this->inputContainerHtmlOptions['class'] .= ' ' . $this->inputContainerCssClass;
		}

		if (!isset($this->headerHtmlOptions['class'])) {
			$this->headerHtmlOptions = array_merge($this->headerHtmlOptions, ['class' => $this->headerCssClass]);
		} else {
			$this->headerHtmlOptions['class'] .= ' ' . $this->headerCssClass;
		}

		if (!isset($this->inputHtmlOptions['class'])) {
			$this->inputHtmlOptions = array_merge($this->inputHtmlOptions, ['class' => $this->inputCssClass]);
		} else {
			$this->inputHtmlOptions['class'] .= ' ' . $this->inputCssClass;
		}

		if (!isset($this->removeHtmlOptions['class'])) {
			$this->removeHtmlOptions = array_merge($this->removeHtmlOptions, ['class' => $this->removeCssClass]);
		} else {
			$this->removeHtmlOptions['class'] .= ' ' . $this->removeCssClass;
		}

		if (!isset($this->addHtmlOptions['class'])) {
			$this->addHtmlOptions = array_merge($this->addHtmlOptions, ['class' => $this->addCssClass]);
		} else {
			$this->addHtmlOptions['class'] .= ' ' . $this->addCssClass;
		}
			
		if ($this->models === []) {
			$this->headerHtmlOptions = array_merge($this->headerHtmlOptions, ['style' => 'display:none']);
		}
	}
...
<<<SCRIPT
	...
	$("#{$this->id} .{$this->removeCssClass}").on("click", function(event) {
		event.preventDefault();
...
SCRIPT;
	...
	protected function renderContent()
	{
		foreach ($this->models as $index => $model) {
			...
			$this->controller->renderPartial($this->inputView, ['model' => $model, 'index' => $index]);
	...
	protected function getAddLink()
	{
		...
		if ($this->addTemplate) {
			$addLink = strtr($this->addTemplate, ['{link}' => $addLink]);
		}
		return $addLink;
	}
	...
	protected function getRemoveLinkAndIndexInput($index)
	{
		...
		if ($this->removeTemplate) {
			$removeLink = strtr($this->removeTemplate, ['{link}' => $removeLink]);
		}
		return $removeLink;
	}
}