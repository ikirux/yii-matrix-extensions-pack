...
class UWfile {
    ...
    public function setAttributes($value, $model, $field_varname) {
        ...
        if ($this->_file_instance = CUploadedFile::getInstance($model, $field_varname)) {
            $model->getEventHandlers('onAfterSave')->insertAt(0, [$this, 'processFile']);
            ...
            if (isset($_POST[get_class($model)]['uwfdel'][$field_varname]) 
                && ($_POST[get_class($model)]['uwfdel'][$field_varname])
            ) {
                $model->onAfterSave = [$this, 'processFile'];
                $path = '';
            }
        }
    ...
    public function viewAttribute($model, $field) {
        if ($file = $model->getAttribute($field->varname)) {
    ...
    public function editAttribute($model, $field, $params = []) {
        ...
        . ' ' . CHtml::activeLabelEx($model, '[uwfdel]' . $field->varname, ['label' => UserModule::t('Delete file'), 'style' => 'display:inline;']) : '')
        ;
    }
