...
class Profile extends UActiveRecord
{
		...
		if (!self::$_rules) {
			...
			foreach ($model as $field) {
				...
				if ($field->field_type == 'VARCHAR' || $field->field_type == 'TEXT') {
					$field_rule = [$field->varname, 'length', 'max' => $field->field_size, 'min' => $field->field_size_min];
				...
				if ($field->other_validator) {
                    ...
                    if ($field->required) {
                        ...
                        $field_rule = [$field->varname, 'date', 'format' => ['yyyy-mm-dd','0000-00-00'], 'allowEmpty' => true];
                    }
			...
			array_push($rules, [implode(',', $decimal), 'match', 'pattern' => '/^\s*[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?\s*$/']);
			self::$_rules = $rules;
		}
	...
	public function attributeLabels()
	{
		...
		foreach ($model as $field) {
			$labels[$field->varname] = ((Yii::app()->getModule('user')->fieldsMessage) ? UserModule::t($field->title, [], Yii::app()->getModule('user')->fieldsMessage) : UserModule::t($field->title));
		}
