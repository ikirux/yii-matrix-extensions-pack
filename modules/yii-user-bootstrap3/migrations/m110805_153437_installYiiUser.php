...
class m110805_153437_installYiiUser extends CDbMigration
{
	...
	public function safeUp()
	{
        ...
        switch ($this->dbType()) {
            case "mysql":
                $this->createTable(Yii::app()->getModule('user')->tableUsers, [
                    "id" => "pk",
                ...
                $this->createTable(Yii::app()->getModule('user')->tableProfiles, [
                    'user_id' => 'pk',
                    'first_name' => 'string',
                    'last_name' => 'string',
                ], $this->MySqlOptions);
                ...
                $this->createTable(Yii::app()->getModule('user')->tableProfileFields, [
                    "id" => "pk",
        ...
        $this->insert(Yii::app()->getModule('user')->tableUsers, [
            "id" => "1",
        ...
        $this->insert(Yii::app()->getModule('user')->tableProfiles, [
            "user_id" => "1",
            "first_name" => "Administrator",
            "last_name" => "Admin",
        ]);

		$this->insert(Yii::app()->getModule('user')->tableProfileFields, [
            "id" => "1",
            "varname" => "first_name",
            "title" => "First Name",
            "field_type" => "VARCHAR",
    ...
    private function readStdin($prompt, $valid_inputs, $default = '') {
        while((is_array($valid_inputs) && !in_array($input, $valid_inputs)) 
            || !isset($input) || ($valid_inputs == 'is_file' && !is_file($input))
        ) {
            echo $prompt;
