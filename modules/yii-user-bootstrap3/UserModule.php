...
class UserModule extends CWebModule
{
	...
	public static function getAdmins() {
		if (!self::$_admins) {
			...
			self::$_admins = ($return_name)?$return_name : [''];
		}
	...
	public static function user($id = 0, $clearCache = false) {
		...
		if ($id) {
            if (!isset(self::$_users[$id]) || $clearCache) {
                self::$_users[$id] = User::model()->with(['profile'])->findbyPk($id);
            }
	...
	public static function getUserByName($username) {
		if (!isset(self::$_userByName[$username])) {
			$_userByName[$username] = User::model()->findByAttributes(['username' => $username]);
		}
