...
class LanguageController extends Controller
{
	...
	*/
	...
	public function actionListPdf()
	{
		...
		$pdf->SetTitle(Yii::t('default', 'Listado') . ' Languages');
		$pdf->SetSubject(Yii::t('default', 'Listado') . ' Languages');
		...
		$html = "<h1><?= Yii::t('default', 'Listado'); ?> Languages</h1>
		<table border=\"1\">
		<tr>
		...
		$pdf->Output('listado_Languages.pdf', 'I');
		Yii::app()->end();
	}
	...
	public function actionListExcel()
	{
		...
		$excel->getProperties()->setCreator(Yii::app()->name)
			...
			->setTitle(Yii::t('default', 'Listado') . ' Languages')
			->setSubject(Yii::t('default', 'Listado') . ' Languages')
			->setDescription("")
			->setKeywords("")
			->setCategory("");
		...
		$excel->setActiveSheetIndex(0)
			...
			->setCellValue(chr($letterCode++) . $index, $model->getAttributeLabel('name'))
			->setCellValue(chr($letterCode++) . $index, $model->getAttributeLabel('code'));
		...
		foreach($iterator as $element) {
			...
			$excel->setActiveSheetIndex(0)
				...
				->setCellValue(chr($letterCode++) . $index, $element->name)
				->setCellValue(chr($letterCode++) . $index, $element->code);
		}

		// Rename worksheet
		$excel->getActiveSheet()->setTitle(Yii::t('default', 'Listado') . ' Languages');
		...
		header('Content-Disposition: attachment;filename="list_Languages.xls"');
